...
import gettext
gettext.install('pathological', './locale')
...
for arg in sys.argv[1:]:
	if arg == '-s':
		...
		print(_("Usage: %s [-cb] [-f] [-s] [highscores-file]\n") % sys.argv[0])
		sys.exit(1)
	else:
		highscores_file = arg
...
life_marble_size = 16
screen_width = 36 + tile_size * horiz_tiles + marble_size
screen_height = 20 + tile_size * vert_tiles + marble_size
...
def load_image(name, colorkey=-1, size=None):
	...
	try:
		...
		print(_('Cannot load image:'), fullname)
...
def load_sound(name, volume=1.0):
	...
	try:
		...
		print(_('Cannot load sound:'), fullname)
		return NoneSound()
...
def start_music(name, volume=-1):
	...
	if not pygame.mixer or not pygame.mixer.music:
		print(_("Background music not available."))
		return
	fullname = os.path.join('music', name)
	try:
		pygame.mixer.music.load(fullname)
	except pygame.error as message:
		print(_('Cannot load music:'), fullname)
		return
	music_loaded = 1
...
def toggle_fullscreen():
	# pygame 2 removed display.toggle_fullscreen(); signal a warm restart
	# instead, which the main loop handles by rebuilding all surfaces.
	return 0
...
def my_tick( frames_per_sec):
	...
	if next_frame < now:
		...
		pygame.time.delay( int(next_frame) - now)
...
def load_images():
	Marble.images = []
	for i in range(9):
		Marble.images.append( load_image('marble-'+str(i)+cbext, -1,
	...
	for i in range(16):
		...
		path = load_image('path-'+str(i)+'.png', -1, (tile_size,tile_size))
		tile.blit( path, (0,0))
		Tile.plain_tiles.append( tile)
		Tile.tunnels.append(load_image('tunnel-'+str(i)+'.png',
	...
	Painter.images = []
	for i in range(8):
		Painter.images.append( load_image('painter-'+str(i)+cbext, -1,
			(tile_size,tile_size)))

	Filter.images = []
	for i in range(8):
		Filter.images.append( load_image('filter-'+str(i)+cbext, -1,
			(tile_size,tile_size)))
	...
	for i in range(4):
		...
		for j in range(4):
			if i == j: Switch.images[i].append( None)
				...
				'switch-'+str(i)+str(j)+'.png',-1,(tile_size,tile_size)))
...
def set_video_mode():
	...
	pygame.display.set_icon(icon) # Needed both before and after set_mode
	pygame.display.set_caption(_('Pathological'))
...
class Wheel(Tile):
	...
	def draw_back(self, surface):
		...
		if self.spinpos:
			...
			for i in range(4):
				holecenter = holecenters[self.spinpos % wheel_steps][i]
	...
	def draw_back(self, surface):
		...
		for i in range(self.current,3):
			surface.blit( self.smallmarbles[self.marbles[i]],
				(self.rect.centerx-marble_size//2,
				 self.rect.top+int(round(tile_size/30.0))+
				 	int(tile_size/3.1)*i))
		return 1
...
class Board:
	def __init__(self, game, pos):
		...
		if( game.level < game.numlevels):
			...
			hash = hashlib.md5((str(game.gamestart)+"/"+str(game.level)).encode()).digest()
			hashval = (hash[0] + (hash[1] << 8) + \
				(hash[2] << 16) + (hash[3] << 24)) & 32767;
		...
		self.launch_timer_text = launch_timer_font.render(
			str(self.launch_timer), 1, (255,255,255))
		...
		# Draw the Backdrop
		backdrop = load_image(self.backdrop, None,
		...
		# Draw the board name
		board_name = str(self.game.level+1) + " - " + self.name
	...
	def draw_back(self, dirty_rects):
		...
		# Draw the score
		text = _("Score: ")+("00000000"+str(self.game.score))[-8:]
		...
		text = str(time_remaining//60)+":"+("00"+str(time_remaining%60))[-2:]
		text = info_font.render( text, 1, (0,0,0))
		rect = text.get_rect()
		...
		text = str(num_marbles)+"/"+str(self.live_marbles_limit)
	...
	def affect_marble(self, marble):
		...
		if cy < 0 and marble.direction != 2:
			...
			if tile_xr == tile_size // 2 and (tile.paths & 1):
				if isinstance( tile, Wheel):
					if tile.spinpos or tile.marbles[0] != -3: return
					tile.marbles[0] = -2
					marble.direction = 2
					self.launch_marble()
	...
	def _load(self, circuit, level):
		...
		while j < vert_tiles:
			...
			# Stop if we've reached the end of the level design
			if j > 0 and line[0] == '+': break
			...
			if line[0] != '|':
				...
				elif line[0:9] == 'backdrop=':
					self.backdrop = line[9:-1]

				continue

			i = 0
			while i < horiz_tiles:
				if len(line) < (i+1)*4: break
					...
					if event.key == K_ESCAPE: return -3
						...
						if self.paused:
							if screenshot:
								...
								pause_popup = popup(_('Game Paused'))
						else:
							popdown( pause_popup)
					elif event.key == K_F2:
						toggle_fullscreen()
	...
	def save(self):
		try:
			...
			try:
				...
				print(_("Warning: Can't save highscores:"), message)
				return

		try:
			for i in self.scores:
				f.write( str(i[0])+' '+i[1]+' '+str(i[2])+' '+i[3]+'\n')
			f.close()
		except:
			print(_("Warning: Problem saving highscores."))
	...
	def play(self):
		...
		while 1:
			...
			if rc < 0:
				...
				if rc == -3: message = _('Level Aborted.')
				elif rc == -2: message = _('The board timer has expired.')
				else: message = _('The launch timer has expired.')

				self.lives -= 1
				if self.lives > 0:
					rc = self.board_dialog( message+_('\nClick to try again.'),
						rc != -3)
				elif self.highscores.qualifies( self.score):
					popup(_("Congratulations!\n"+
				...
				message = _('Level Complete!\n')+ \
					_("Bonus for %r%% time remaining: %r\n") % (time_remaining, time_bonus) +\
					_("Bonus for %r%% holes empty: %r\n") % (empty_holes, holes_bonus) + \
                    _('\nClick to continue.')
...
class IntroScreen:
	...
	start_level = 1
	menu_width = 240
	menu_pos = ((screen_width - menu_width)/2, int(screen_height/4.13))
	menu_font_height = 32
	...
	scroller_rect = (10,screen_height-50,screen_width-20,scroller_font_height)
	scroller_text = '   '.join([
		_("Copyright (C) 2003  John-Paul Gignac."),
		_("Soundtrack by Matthias Le Bidan."),
		_("Board designs contributed by Mike Brenneman and Kim Gignac."),
		_("To contribute your own board designs, see the website: http://pathological.sourceforge.net/"),
		_("Logo by Carrie Bloomfield."),
		_("Other graphics based on artwork by Mike Brenneman."),
		_("Project motivated by Paul Prescod."),
		_("Thanks to all my friends who helped make this project a success!"),
		_("This program is free software; you can redistribute it and/or modify it under the terms of the GNU General Public License. See the LICENSE file for details."),
        ])
	...
	def draw_menu(self):
		...
		levelt = self.menu_font.render(_("(Lvl. %d)") % IntroScreen.start_level,
					       1, self.menu_color)
	...
	hs_font_height = 24
	hs_width = 320
	hs_pos = ((screen_width-hs_width)//2, int(screen_height / 5.26))
	hs_margin = 8
	hs_column_margin = 70
	hs_score_width = 70
	hs_level_width = 24
	...
	def draw_highscores(self):
		...
		tname = self.hs_font.render(_("Name"), 1, self.hs_heading_color)
		tscore = self.hs_font.render(_("Score"), 1, self.hs_heading_color)
		tlevel = self.hs_font.render(_("Level"), 1, self.hs_heading_color)
...
#					(x - 30, y, self.hs_width + 30, self.hs_font_height))
			...
			number = self.hs_font.render(str(j+1)+'.',1,numcolor)
			...
			level = self.hs_font.render( str(i[2]), 1, color)
			self.screen.blit( level, (x + level_right - level.get_width(), y))
			score = self.hs_font.render( str(i[0]), 1, color)
...
def setup_everything():
	...
	if not pygame.font: print(_('Warning, fonts disabled'))
	if not pygame.mixer: print(_('Warning, sound disabled'))
